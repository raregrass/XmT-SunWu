...
const int WM_keyboard = USER + 110;
const int WM_mouse[6] = { WM_mouse1 , WM_mouse2 , WM_mouse3 , WM_mouse4 };
...
/*	//求最终的平均掌心位置 
	...
	HWND hWnd = FindWindow( NULL , _T("客户端") );	
	if( hWnd != NULL ){
		PostMessage( hWnd , WM_mouse[contour_number] , min_circle_center2.x , min_circle_center2.y );
	}
	else{	
		printf("No client !");
	}         
/*    //求平均的手掌大小  
    ...
    cvCircle( frame , min_circle_center2 , cvRound(radius*1.5) , CV_RGB(200,100,200) , 1 , 8 , 0 );  //画出指缝排除圆
    ...
    { 
		...
		if( (int)finger_length > cvRound(radius*1.5) ) 
        { 
			finger_count += 1;    
